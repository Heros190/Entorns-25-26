...
def child():
    ...
    if (not user_id and not user_id.is_integer): #validacio de dades entrada
        return jsonify(asdict(response)),400

    user_id = int(user_id)
    u=User(id=user_id, username="",passwdord="", email="", idrole=1, token="")
    listChilds=ChilDao.getChilds(u)
